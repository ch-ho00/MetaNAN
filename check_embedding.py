...
if not skip_forward:
    ...
    torch.save(embeddings, f'./results/{prefix}_embedding_{num_sample}.pt')
    ...
    with open(f'./results/{prefix}_hparams_{num_sample}.pkl', 'wb') as fp:
        pickle.dump(all_hparams, fp)

with open(f'./results/{prefix}_hparams_{num_sample}.pkl', 'rb') as fp:
...
for gain_level in [0, 1, 8, 16, 20]: #,1, 20, 16, 8 4,
    ...
    for idx in tqdm(sampled_idxs, desc=f'Retrieving close embedding at {gain_level}'):
        rgb_d1, parsed_hparams = val_dataset.get_singleview_param(idx, eval_gain=gain_level)
    ...
    for k in hparams.keys():        
        print(k)
        if k == 'white_level' or k == 'gain_level' :
            continue
        if k == 'kernel':
        ...
        disp.plot()
        plt.savefig(f'./results/{prefix}_gain{gain_level}_{num_sample}_{k}_confusion.png')
        plt.clf()
    print()        
