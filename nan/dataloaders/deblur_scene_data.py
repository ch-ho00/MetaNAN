...
class DeblurSceneDataset(NoiseDataset, ABC):
    ...
    def get_multiview_item(self, idx):
        ...
        rgb, camera, src_rgbs, src_cameras, rgb_noisy = self.apply_transform(rgb, camera, src_rgbs, src_cameras, rgb_noisy)

        gt_depth = 0
        depth_range = self.final_depth_range(depth_range)
        return self.create_deblur_scene_batch_from_numpy(rgb, camera, rgb_file, src_rgbs, src_cameras, depth_range, gt_depth=gt_depth, eval_gain=eval_gain, rgb_noisy=rgb_noisy)
...
class DeblurSceneTestDataset(DeblurSceneDataset):
    ...
    def apply_transform(self, rgb, camera, src_rgbs, src_cameras, rgb_noisy):
        if self.mode is Mode.train and self.random_crop:
            ...
            crop_h = 350
            crop_w = 550
            rgb, camera, src_rgbs, src_cameras, rgb_noisy = random_crop(rgb, camera, src_rgbs, src_cameras,
                                                             (crop_h, crop_w), rgb_noisy=rgb_noisy)

        if self.mode is Mode.train and np.random.choice([0, 1]):
            rgb, camera, src_rgbs, src_cameras, rgb_noisy = random_flip(rgb, camera, src_rgbs, src_cameras, rgb_noisy=rgb_noisy)

        return rgb, camera, src_rgbs, src_cameras, rgb_noisy
