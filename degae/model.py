...
class DegAE(nn.Module):
    ...
    def create_optimizer(self):
        params_list = [{'params': self.encoder.parameters(),                        'lr': self.args.lrate_feature * (1e-2 if self.args.ft_training else 1)},
                       {'params': self.degrep_extractor.degrep_conv.parameters(),   'lr': self.args.lrate_feature * (1e-2 if self.args.ft_training else 1)},
                       {'params': self.degrep_extractor.degrep_fc.parameters(),     'lr': self.args.lrate_feature * (1e-2 if self.args.ft_training else 1)},
                       {'params': self.decoder.parameters(),                        'lr': self.args.lrate_feature * (1e-2 if self.args.ft_training else 1)},                       
                       ]
